...
<header class="h-[57px] shrink-0 bg-white border-b border-slate-100 flex items-center gap-4 px-5">
    ...
    <div class="ml-auto flex items-center gap-3">
        ...
        <!-- Search bar -->
        <div class="hidden md:flex items-center gap-2 px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-400 w-52 cursor-pointer hover:border-slate-300 transition">
            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
            </svg>
            <span class="flex-1 text-xs">Search category...</span>
            <span class="text-[10px] font-mono bg-slate-200 text-slate-500 px-1.5 py-0.5 rounded">Ctrl+K</span>
        </div>
        ...
        <button class="relative p-2 rounded-lg text-slate-500 hover:bg-slate-100 transition">
            ...
            <span class="absolute top-1 right-1 w-4 h-4 bg-red-500 text-white text-[9px] font-bold rounded-full flex items-center justify-center leading-none">3</span>
        </button>
        ...
        <div class="flex items-center gap-2.5 pl-3 border-l border-slate-100">
            ...
            <div class="hidden md:block leading-none">
                <p class="text-sm font-semibold text-slate-800">{{ auth()->user()->name ?? 'John Doe' }}</p>
